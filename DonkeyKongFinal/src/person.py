...
class person(pygame.sprite.Sprite):
    ...
    def checkWallcollision(self):
        ...
        if self.wallList != None:
            WallCollsionlist = pygame.sprite.spritecollide(self,
                    self.wallList, False)
  ...
  #       Checking vertical hits
        ...
        if self.wallList != None:
            WallCollisionlist = pygame.sprite.spritecollide(self,
                    self.wallList, False)
    ...
    def checkLadders(self):
        ...
        if self.ladderList != None:
            LadderTouch = pygame.sprite.spritecollide(self,
                    self.ladderList, False)
            for ladders in LadderTouch:
                if len(LadderTouch) > 0 and ladders.rect.left \
                    <= self.rect.left and ladders.rect.right \
                    >= self.rect.right:
        ...
        for ladder in self.ladderposition:
            ...
            if self.ladderposition[iterator][1] - 2 <= self.rect.bottom \
                <= self.ladderposition[iterator][1] + 1 \
                and self.rect.left >= self.ladderposition[iterator][0] \
                and self.rect.right <= self.ladderposition[iterator][0] \
                + 19:
    ...
    def postopladders(self):
        ...
        for i in self.ladderposition:
            ...
            if self.ladderposition[iterator][1] + 1 == self.rect.bottom:
                self.rect.bottom -= 1
            if self.rect.bottom == self.ladderposition[iterator][1] \
                and self.rect.left >= self.ladderposition[iterator][0] \
                and self.rect.right <= self.ladderposition[iterator][0] \
                + 25:
                return False
    ...
    def Jump(self):
        ...
        print 'Jump'
        self.rect.y += 1
        Safeland = pygame.sprite.spritecollide(self, self.wallList,
                False)
        self.rect.y -= 1
...
class player(person):
    ...
    def __init__(
        ...
        self.marioright = pygame.transform.scale(self.marioright,
                (size, size))
        ...
        self.mariowalkright = \
            pygame.transform.scale(self.mariowalkright, (size, size))

        self.mariowalkleft = \
            pygame.transform.scale(self.mariowalkright, (size, size))
        self.mariowalkleft = pygame.transform.flip(self.mariowalkright,
                1, 0)

        self.marioleft = pygame.transform.scale(self.marioright, (size,
                size))
    ...
    def update(self):
        ...
        self.fireballscheck()
        person.update(self)
        if self.destination[0] - size / 2 <= self.rect.x \
            <= self.destination[0] + size / 2 and self.destination[1] \
            - size / 2 <= self.rect.y <= self.destination[1] + size / 2:
            self.gameover = True
    ...
    def collectCoins(self):
        if self.coinList != None:
            CoinCollection = pygame.sprite.spritecollide(self,
                    self.coinList, True)
    ...
    def ladderscheck(self):
        if self.topladderList != None:
            Topcollision = pygame.sprite.spritecollide(self,
                    self.topladderList, False)
    ...
    def fireballscheck(self):
        fireball = pygame.sprite.spritecollide(self,
                self.fireballsList, False)
...
class donkey(person):
    ...
    def __init__(
        ...
        self.donkeyfront = pygame.image.load('../images/donkeyfront.png'
                )#.convert_alpha()
        self.donkeyfront = pygame.transform.scale(self.donkeyfront,
                (size, size))
